		 	...
		 	<!-- end of allow to import file or stage transfer -->
		 	 ...
		 	 <@s.if test = "%{datasets != null & datasets.size >0}">
			 	...
			 	<table class="table_data" >
			 		  <tr class="bg_grey_tr">
			 	 		<td width="360" height="20"><center><b>Name</b></center></td>
			 	 		<td width="160">&nbsp;</td>
			 	 	</tr>
			 	 				...
			 	 				<div class="inline_td_div"> 
			 	 					<div id='confirm-dialog'>
										<@s.if test="%{permissionBean.viewAllowed}">
					 	   				...
					 	   				<!--
					 	   				<a href="${base}/${viewDatasetLink}?dataset.id=<@s.property value='#ds.id' />&collection.id=<@s.property value='collection.id' />&collection.owner.id=<@s.property value='collection.owner.id' />&viewType=${viewType}" title="Dataset - ${ds.name}" class="greybox">View Data </a> &nbsp;
					 	   				-->
