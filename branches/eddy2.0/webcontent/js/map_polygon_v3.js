function PolygonCreator(map, precoords) {
	// create pen to draw on map
	this.map = map;
	this.pen = new Pen(this.map);

	var thisOjb = this;
	// draw pre set pots and draw a polygon region
	if (precoords != null) {
		for ( var i = 0; i < precoords.length; i++) {
			thisOjb.pen.draw(precoords[i]);
		}
		thisOjb.pen.drawPolygon(thisOjb.pen.listOfDots);
	}
...
function Pen(map) {
	...
	this.draw = function(latLng) {
		if (null != this.polygon) {
			alert("Press 'Clear' to draw another polygon");
		} else {
	...
	// get plots data, return longitude latitude pairs separated by space
...
/*
 * Child of Pen class polygon class
 */
function Polygon(listOfDots, map, pen, color) {
	...
	this.addListener = function() {
		...
		// alert("data: " + this.coords);
		var coordstr='';
		for(var i=0; i< this.coords.length; i++){
			coordstr += this.coords[i].lng() + "," + this.coords[i].lat() + ' ';
		}
		$("#spatialcvg").val(coordstr);
...
/*
 * Child of Polygon class Info Class
 */
