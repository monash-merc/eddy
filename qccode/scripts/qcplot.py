...
def plottimeseries(cf,nFig,SeriesList,dsa,dsb,si,ei):
    ...
    L1XArray = numpy.array(dsa.series['DateTime'][si:ei])
    L2XArray = numpy.array(dsb.series['DateTime'][si:ei])
    XAxMin = min(L2XArray)
    XAxMax = max(L2XArray)
    ...
    for ThisOne, n in zip(SeriesList,range(nGraphs)):
        if ThisOne in dsa.series.keys() and ThisOne in dsb.series.keys():
            ...
            hr2_ax = fig.add_axes([hr2_XAxOrg,YAxOrg,hr2_XAxLen,ts_YAxLen])
            hr2_ax.hold(False)
            nSd = None
