...
class qcgui(Tkinter.Frame):
    ...
    def do_l2qc(self):
        ...
        self.update_startenddate(str(self.ds1.series['DateTime'][0]),str(self.ds1.series['DateTime'][-1]))
    ...
    def do_l3qc(self):
        ...
        self.update_startenddate(str(self.ds2.series['DateTime'][0]),str(self.ds2.series['DateTime'][-1]))
        if self.ds2.globalattributes['SiteName']=='AdelaideRiver':
            ...
            self.do_progress(text='Finished saving L3 QC & Corrected NetCDF data')              # tell the user we are done
            log.info(' Finished saving L3 QC & Corrected NetCDF data')
        #elif self.ds2.globalattributes['SiteName']=='DalyPasture':
            #self.do_progress(text='Doing L3 QC & Corrections: Daly Pasture ...')
            #self.ds3 = qcls.l3qc_DalyPasture(self.cf,self.ds2)
            #self.do_progress(text='Finished L3: Daly Pasture')
            #log.info(' Finished L3: Daly Pasture')
            #self.do_progress(text='Saving L3 QC & Corrected NetCDF data ...')                     # put up the progress message
            #qcio.nc_write_series(self.cf,self.ds3,'L3')                   # save the L3 data
            #if 'OFL2' in self.cf['Output']:
                #qcio.nc_write_OzFlux_series(self.cf,self.ds3,'L3_Corrected')
            #self.do_progress(text='Finished saving L3 QC & Corrected NetCDF data')              # tell the user we are done
            #log.info(' Finished saving L3 QC & Corrected NetCDF data')
        elif self.ds2.globalattributes['SiteName']=='DalyRegrowth':
            self.do_progress(text='Doing L3 QC & Corrections: Daly Regrowth ...')
            self.ds3 = qcls.l3qc_DalyRegrowth(self.cf,self.ds2)
            self.do_progress(text='Finished L3: Daly Regrowth')
            log.info(' Finished L3: Daly Regrowth')
            self.do_progress(text='Saving L3 QC & Corrected NetCDF data ...')                     # put up the progress message
            qcio.nc_write_series(self.cf,self.ds3,'L3')                   # save the L3 data
            if 'OFL2' in self.cf['Output']:
                qcio.nc_write_OzFlux_series(self.cf,self.ds3,'L3_Corrected')
            self.do_progress(text='Finished saving L3 QC & Corrected NetCDF data')              # tell the user we are done
            log.info(' Finished saving L3 QC & Corrected NetCDF data')
        #elif self.ds2.globalattributes['SiteName']=='DalyUncleared':
            #self.do_progress(text='Doing L3 QC & Corrections: Daly Uncleared ...')
            #self.ds3 = qcls.l3qc_DalyUncleared(self.cf,self.ds2)
            #self.do_progress(text='Finished L3: Daly Uncleared')
            #log.info(' Finished L3: Daly Uncleared')
            #self.do_progress(text='Saving L3 QC & Corrected NetCDF data ...')                     # put up the progress message
            #qcio.nc_write_series(self.cf,self.ds3,'L3')                   # save the L3 data
            #if 'OFL2' in self.cf['Output']:
                #qcio.nc_write_OzFlux_series(self.cf,self.ds3,'L3_Corrected')
            ...
            self.do_progress(text='Finished saving L3 QC & Corrected NetCDF data')              # tell the user we are done
            log.info(' Finished saving L3 QC & Corrected NetCDF data')
        #elif self.ds2.globalattributes['SiteName']=='SturtPlains':
            #self.do_progress(text='Doing L3 QC & Corrections: Sturt Plains ...')
            #self.ds3 = qcls.l3qc_SturtPlains(self.cf,self.ds2)
            #self.do_progress(text='Finished L3: Sturt Plains')
            #log.info(' Finished L3: Sturt Plains')
            #self.do_progress(text='Saving L3 QC & Corrected NetCDF data ...')                     # put up the progress message
            #qcio.nc_write_series(self.cf,self.ds3,'L3')                   # save the L3 data
            #if 'OFL2' in self.cf['Output']:
                #qcio.nc_write_OzFlux_series(self.cf,self.ds3,'L3_Corrected')
    ...
    def do_l4qc(self):
        ...
        self.update_startenddate(str(self.ds3.series['DateTime'][0]),str(self.ds3.series['DateTime'][-1]))
    ...
    def do_plotL1L2(self):
        ...
        for nFig in self.cf['Plots'].keys():
            ...
            si = qcutils.GetDateIndex(self.ds1.series['DateTime'],self.plotstartEntry.get(),0)
            ei = qcutils.GetDateIndex(self.ds1.series['DateTime'],self.plotendEntry.get(),-1)
    ...
    def do_plotL3L3(self):
        ...
        for nFig in self.cf['Plots'].keys():
            SeriesList = eval(self.cf['Plots'][str(nFig)]['Variables'])
            si = qcutils.GetDateIndex(self.ds3.series['DateTime'],self.plotstartEntry.get(),0)
            ei = qcutils.GetDateIndex(self.ds3.series['DateTime'],self.plotendEntry.get(),-1)
